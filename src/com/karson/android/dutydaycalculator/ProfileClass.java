...
public class ProfileClass {
	...
	// {+-1:before or after alert time; hours; minutes}

	public ProfileClass(String profName) {
		name = profName;
		rowNames.add("Alert");
		rowValues.add(new int[] { 1, 0, 0 });
	}
	...
	public int getIndexOfAlert() {
		return rowNames.indexOf("Alert");
	}
	...
	public String getRowDeltaTimeString(int index) {
		String result = "";
		if (rowValues.get(index)[AFTERINDEX] > 0) {
			result += "+";
		} else {
			result += "-";
		}
		String minutes = "" + rowValues.get(index)[MINUTEINDEX];
		if (rowValues.get(index)[MINUTEINDEX] < 10) {
			minutes = "0" + minutes;
		}
		result += rowValues.get(index)[HOURINDEX] + "+" + minutes;
		return result;
	}

	// Return time resulting from adding(or subtracting) row value and baseTime
	public Calendar getRowTime(Calendar baseTime, int index) {
		Calendar output = (Calendar) baseTime.clone();
		output.add(
				Calendar.HOUR_OF_DAY,
				(rowValues.get(index)[AFTERINDEX] * rowValues.get(index)[HOURINDEX]));
		output.add(
				Calendar.MINUTE,
				(rowValues.get(index)[AFTERINDEX] * rowValues.get(index)[MINUTEINDEX]));
		return output;
	}
